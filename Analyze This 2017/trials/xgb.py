...
def analyse(classifier):
	...
	# for item in Ypred_prob:
	# 	Yts_dum=pd.get_dummies(Yts)
	# 	mse=metrics.mean_squared_error(Yts_dum.values,item)
...
def generate_submission(pred,classes):
	...
	sub=sub[0:1000]
	sub.to_csv("./xgb/result_350_0.08.csv", header=False, index=False)
	return sub

if __name__ == '__main__':
	# weight={'Supp': 5, 'Elite': 5, 'Credit': 5,'NoInv' :1}
	# forest=RandomForestClassifier(n_estimators=500, criterion='entropy',random_state=0,class_weight=weight)
	# # forest=GradientBoostingClassifier(learning_rate=0.15, n_estimators=300, max_depth=13, min_samples_split=950, random_state=0, verbose=1)
	...
	# le_y = LabelEncoder()
	# dfy = le_y.fit_transform(dfy)
	# modelfit(forest, dfx, dfy[0])